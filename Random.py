...
#%% 参数设置
"""场景范围"""
Length = 300    #正方形区域，边长500(m),用于产生无人机的初始位置


"""节点/波束数量"""
UAV_Num = 10    #无人机群数量
Beam_Num = 2   #一个微基站可以产生的波束数量上限，每个波束主瓣的宽度为30度
Sec_Num = 8    #微基站扇区数量，由波束宽度为45度可得到
...
'''动作空间计算'''
Sec_list = [(i+1) for i in range(Sec_Num)]  #1,2,...,24表示所有扇区的编号
Act_Space = list(combinations(Sec_list, Beam_Num)) #所有可能的扇区组合
...
class UAV:
    ...
    def act(self, slot_num): 
        flag = slot_num % len(Act_Space)
        return Act_Space[flag]
...
for com_r in range(Comm_Num):
    ...
    for slot_i in range(Slot_Num): 
        ...
        for uav_j in range(UAV_Num):
            ...
            act_uav_j = UAV_List[uav_j].act_r()
            UAV_List[uav_j].action = act_uav_j
        ...
        for link_j in range(len(Link_Set)):
            ...
            if (Link_Sec[link_j][0] in Act_all_node[node_c]) and (Link_Sec[link_j][1] in Act_all_node[node_d]): #波束对齐
                ...
                if nei_num_1 < 1:
                   UAV_List[node_c].idle_num += 1
                if nei_num_2 < 1:
                   UAV_List[node_d].idle_num += 1
                ...
                col_pro = np.log(nei_num_sum - 1)/np.log(6) #碰撞概率
        ...
        #计算二跳邻居表
        for link_i in range(len(Link_Set_Exi)):
            node_1 = Link_Set_Exi[link_i][0]
            node_2 = Link_Set_Exi[link_i][1]
            for nei_one_i in UAV_List[node_1 - 1].nei_one:
                if nei_one_i[0] == node_2:
                    continue
                else:
                    dis_j, sec_j = Link_Com(UAV_List[node_2 - 1], UAV_List[nei_one_i[0] - 1])
                    if ((dis_j < Dis_Thr) and ([nei_one_i[0], sec_j] not in UAV_List[node_2 - 1].nei_one)) and ([nei_one_i[0], sec_j] not in UAV_List[node_2 - 1].nei_two):
                        UAV_List[node_2 - 1].nei_two.append([nei_one_i[0], sec_j])
                    
            for nei_one_j in UAV_List[node_2 - 1].nei_one:
                if nei_one_j[0] == node_1:
                    continue
                else:
                    dis_k, sec_k = Link_Com(UAV_List[node_1 - 1], UAV_List[nei_one_j[0] - 1])
                    if ((dis_k < Dis_Thr) and ([nei_one_j[0], sec_k] not in UAV_List[node_1 - 1].nei_one)) and ([nei_one_j[0], sec_k] not in UAV_List[node_1 - 1].nei_two):
                        UAV_List[node_1 - 1].nei_two.append([nei_one_j[0], sec_k])
...
txtName = "Link_Percent_Random.txt"
f = open(txtName, "a+")
