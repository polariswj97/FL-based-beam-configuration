...
class UAV:
    def __init__(self, name):
        ...
        self.Net_Weight_A = [0 for m in range(NNet_Layer_Num)]  
        self.Net_Weight_Grad_A = [0 for n in range(NNet_Layer_Num)]  
        self.Net_Weight_Grad_A_Set = []  
        
        self.Net_Weight_C = [0 for m in range(NNet_Layer_Num)]
        self.Net_Weight_Grad_C = [0 for n in range(NNet_Layer_Num)]
        self.Net_Weight_Grad_C_Set = []  
    ...
    def Par_Init(self, Par_W_A, Par_W_C):    
        Flag_g = 0
...
def FLBC():
    ...
    UAV_NNet_Weight_A_List = [0 for p in range(UAV_Num)]        
    UAV_NNet_Weight_C_List = [0 for p in range(UAV_Num)]        
    ...
    def Par_Poly(P_A, P_C): 
        for i in range(NNet_Layer_Num):
            Par_Temp = 0 
    ...
    for com_r in range(Comm_Num):
        ...
        if com_r > 0:
            for uav_k in UAV_List:
                uav_k.Par_Init(UAV_Par_A_List, UAV_Par_C_List) 
        ...
        for uav_n in UAV_List:
            uav_n.Extract_Par_A()
            uav_n.Extract_Par_C()      
            UAV_NNet_Weight_A_List[flag_j] = uav_n.Net_Weight_A       
            UAV_NNet_Weight_C_List[flag_j] = uav_n.Net_Weight_C               
            flag_j = flag_j + 1    
        Par_Poly(UAV_NNet_Weight_A_List, UAV_NNet_Weight_C_List) 
